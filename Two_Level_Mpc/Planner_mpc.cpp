...
Planner_mpc::Planner_mpc(GRBEnv *planner_env)
{
	env = planner_env;
...
void Planner_mpc::initialize(double Vx, vector<Point> &rBound, vector<Point> &lBound)
{
	vx = Vx;
	...
	H(NINPUT*NC + 1, NINPUT*NC + 1) = P_Planner.WeightDummy[1];
	...
	obj_h += P_Planner.WeightDummy[0] * vars[NC];

	/* input rate constraint */
	model->addConstr(vars[0], '<', P_Planner.DUMAX);
	model->addConstr(vars[0], '>', P_Planner.DUMAX);
	for (i = 0; i < NC - 1; i++)
	{
		GRBLinExpr lhs = 0;
		lhs += vars[i] - vars[i + 1];
		model->addConstr(lhs, '<', P_Planner.DUMAX);
		model->addConstr(lhs, '>', -P_Planner.DUMAX);
	}
	...
	loadBound(lBound, leftBound, r_veh, "left");
	loadBound(rBound, rightBound, r_veh, "right");
...
void Planner_mpc::step(CAR_STATE *currentState, Path & planned_path, MPC &tracker, vector<Path> &recorder)
{
	...
	{
		currentPos = planned_path.beginState.first;
		kappa = planned_path.beginState.second;
	}
	else
	{
		currentPos = Point(currentState->dPos[0], currentState->dPos[1], currentState->dAng[2]);
	}
...
//	VectorXd w = VectorXd::Ones(cols) * theta_r;            // curved road considered later.
	...
	/* set model objective */
	RowVectorXd f = 2 * (Phi * planner_z + Ew * w).transpose() * Q * Gamma;  
	//f.head(NC) = 2 * (Phi*planner_z + Ew * w - zRef).transpose()*Q*Gamma;  // vector uses head(), tail(), segment(p, q).
	...
	if (!isObsDetected && currentState->cObsDisp == 1) isObsDetected = true;
...
void Planner_mpc::threadStep(CAR_STATE * p_DS_to_C, Path & planned_path, MPC &tracker, vector<Path> &recorder)
{
	...
	while (!terminate)
	{
		if (tracker.terminate || s_index >= ref_path.size())
		{
			terminate = true;
			break;
		}
