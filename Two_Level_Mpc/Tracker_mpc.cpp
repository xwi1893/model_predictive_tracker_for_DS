...
void Tracker_mpc::initialize()
{
	...
	As << -2 * (Car.CF + Car.CR) / (Car.MASS*vx), 2 * (Car.CR*Car.LRAXIS - Car.CF*Car.LFAXIS) / (Car.MASS*vx*vx) - 1, 0, 0,
		...
		0, 1, 0, 0,
		vx, 0, vx, 0;
	Bs << 2.6 * Car.CF / (Car.MASS*vx), 2.6 * Car.CF*Car.LFAXIS / Car.IZ, 0, 0;
	Es << 0, 0, -vx, 0;
...
// load planned_path to ref_path
	...
	beginStateMutex.lock();
	isDeviate_ref = (abs(d_offset) > REPLANERROR) ? true : false;
	beginStateMutex.unlock();

	/* calculate beginState */
	if (ref_path.size() > 0)
	{
		Point beginStatePoint = ref_path.linear_interpolation(currentS, s_index);
		beginStateMutex.lock();
		beginState.first = beginStatePoint;
		beginState.second = beginStateKappa;
		beginStateMutex.unlock();
	}
